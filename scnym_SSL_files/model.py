...
class CellTypeCLF(nn.Module):
    ...
    def __init__(
        ...
        self.n_coarse_cell_types = n_coarse_cell_types
        self.n_hidden = n_hidden
        ...
        self.classif = nn.Linear(self.n_hidden, self.n_cell_types)
        self.coarse_classif = nn.Linear(self.n_hidden, self.n_coarse_cell_types)
        ...
        self.decoder = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            *hidden_layers,
            nn.Linear(self.n_hidden, self.n_genes),
        )
        return
    ...
    def forward(
        ...
        pred = self.classif(encoded)
        coarse_pred = self.coarse_classif(encoded)
        decoded = self.decoder(encoded)

        return pred, decoded, coarse_pred

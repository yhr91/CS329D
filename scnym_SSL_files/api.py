...
CONFIGS['no_ssl2'] = copy.deepcopy(CONFIGS['default'])
CONFIGS['no_ssl2']['ssl_method'] = 'sup_only'
CONFIGS['no_ssl2']['ssl_kwargs']['dan_max_weight'] = 0.0
CONFIGS['no_ssl2']['ssl_kwargs']['dan_ramp_epochs'] = 1
CONFIGS['no_ssl2']['ssl_kwargs']['dan_criterion'] = None
CONFIGS['no_ssl2']['unsup_max_weight'] = 0.0
CONFIGS['no_ssl2']['description'] = (
...
def scnym_train(
    ...
    #full_adata_unlab = full_adata[full_adata.obs['annotations'] == 'Unlabeled']
    unlabeled_labels = pd.Categorical(
        np.array(target_adata.obs['cell'].values),
        categories=np.unique(train_adata.obs[config['groupby']]),
    ).codes

    try:
        # Remove all cells that do not have a coarse label
        coarse_adata = full_adata[~full_adata.obs['coarse_y'].isna()]

        coarse_X = utils.get_adata_asarray(coarse_adata)
        coarse_y = pd.Categorical(
        np.array(coarse_adata.obs['coarse_y']),
        categories=np.unique(coarse_adata.obs['coarse_y']),
        ).codes

        # Note this requires y and coarse_y to be same length
        coarse_map = {x: y for x, y in zip(y, coarse_y)}
    except:
        pass
    ...
    if 'scNym_split' not in adata.obs_keys():
        ...
        coarse_traintest_idx = np.random.choice(
            coarse_X.shape[0],
            size=int(np.floor(0.9*coarse_X.shape[0])),
            replace=False
        )
        coarse_val_idx = np.setdiff1d(np.arange(coarse_X.shape[0]),
                               coarse_traintest_idx)
    else:
        train_idx = np.where(
    ...
    acc, loss = main.fit_model(
        ...
        coarse_traintest_idx=coarse_traintest_idx,
        coarse_map=coarse_map,
        val_idx=val_idx,
        coarse_val_idx=coarse_val_idx,
...
#         config['trained_model'] = config['out_path']
    ...
    coarse_classes = len(np.unique(coarse_adata.obs['coarse_y']))

    P = predict.Predicter(
        model_weights=model_weights_path,
        n_genes=results['n_genes'],
        n_cell_types=results['n_cell_types'],
        n_coarse_cell_types=config['num_coarse_labels'],
